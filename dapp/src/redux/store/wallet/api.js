...
import { WALLET_STATE, MARKET_TYPE } from "../../types/walletTypes";
import {
  walletConnected,
  setWalletLoading,
  setWalletData2,
  ...
  (wallet, asset, price, callback) => async (dispatch) => {
    try {
      ...
      if (listObj && listObj.datumHash && listObj.txHash) {
        ...
        dispatch(setWalletLoading(false));
        dispatch(setWalletData2(updatedWallet));
      ...
      if (assetUtxo) {
        ...
        const updateObj = await updateListing(
          ...
          await createTxUnspentOutput(
            contractAddress(currentVersion),
            assetUtxo
          ),
          currentVersion,
          latestVersion
...
export const getWalletAssets = async () => {
  ...
  // console.log(utxos);
  const nativeAssets = (
    await Promise.all(utxos.map((utxo) => serializeTxUnspentOutput(utxo)))
  )
    .map((txOutput) => txOutput.output())
