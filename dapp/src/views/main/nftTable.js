...
import { getTopCollections } from "../collections/state"
...
const Collection = () => {
    ...
    const {
        ...
        isSuccess,
        isLoading
    } = getTopCollections(1, 10, filters.category, filters.blockchain, filters.name)
    ...
    useEffect(() => {
        const fetchdata = async () => {
            ...
            if (data && data?.collection?.length > 0) {
                ...
                if (result?.price_percents_one?.length && result?.price_percents_seven?.length)
                    filters.hour === "24h" ? setPrice(result.price_percents_one) : setPrice(result.price_percents_seven)
            }
        }
        fetchdata()
    }, [data])
            ...
            <Grid item lg={12} xs={12}>
                <Grid container classes={{ root: classes.select }} alignItems="flex-end">
                    <Grid item lg={6} justifyContent="left" >
                        <Typography variant="h1">Top Collections</Typography>
                    </Grid>
                                        ...
                                        <StyledTableRow key={index} onClick={() => gotoCollectionDetails(index)} sx={{ cursor: "pointer", "&:hover": { background: "#ddd !important" } }}>
                                            <TableCell component="th" scope="row" className={tclasses.tdbox} sx={{ columnGap: "20px" }}>
                                                ...
                                                <Box className={tclasses.namebox}>
                                                    <span className={tclasses.spanbox}>
                                                        <Box className={tclasses.imgbox} component="img" src={row.coverUrl}>
                                                        </Box>
                                                    </span>
                                                </Box>
                                                <Box sx={{ display: "flex" }}>
                                                    <Typography className={tclasses.name}>{row.name}</Typography>&nbsp;&nbsp;
                                                    ...
                                                    row.floor !== 0 ?
                                                        <Stack direction="row" alignItems="center" justifyContent="right">
                                                            <Box className={tclasses.namebox} sx={{ height: "100%", width: "30px", backgroundColor: "unset !important", border: "none" }}>
                                                                {
                                                                    row?.blockchain === "cardano" ? <Box component="img" className={classes.imgbox} src={cardano.src}></Box> : row?.blockchain === "ethereum" ? <Box component="img" className={classes.imgbox} src={ether.src}></Box> : row?.blockchain === "binance" ? <Box component="img" className={classes.imgbox} src={binance.src}></Box> : row?.blockchain === "polygon" ? <Box component="img" className={classes.imgbox} src={polygon.src}></Box> : row?.blockchain === "solana" ? <Box component="img" className={classes.imgbox} src={solana.src}></Box> : ""
                                                                }
                                                            </Box>
                                                            <Typography className={tclasses.volletter}>&nbsp;&nbsp;{convertFormatedDigit((row?.floor || 0), 2, row?.blockchain)}</Typography>
                                            ...
                                            <TableCell align="right" className={tclasses.ether}>
                                                <Stack direction="column" alignItems="end" justifyContent="center">
                                                    ...
                                                    <Box>
                                                        {
                                                            price?.length ? price[index] ? <TypeItem item={price[index]} /> : <RemoveIcon></RemoveIcon> : <RemoveIcon></RemoveIcon>
                                                        }
                                                    </Box>
                                                </Stack>
                                            </TableCell>
