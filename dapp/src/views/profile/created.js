...
import { Box, Button, Typography, Grid, IconButton, InputBase, Paper, MenuItem, Menu, Divider } from "@mui/material"
...
const CreatedCmp = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            const result = await getAllNfts(currentPage, { createdBy: params.id, name: nameQuery })
            if (result?.nfts) {
                setNfts(result)
            }
        }
    ...
    return (
        <>
                            ...
                            <Grid item key={index} lg={3} md={4} sm={6} xs={12} >
                                <Card onClick={() => { gotoDetails(item) }} className={classes.boxshadow} sx={{ position: "relative" }}>
                                    ...
                                    <Box className={classes.cardlogo} sx={{ position: "absolute", top: "10px", left: "10px", display: "flex", columnGap: "10px", width: "30%" }}>
                                        ...
                                        {
                                            item.blockchain === "cardano" ? <Box component="img" src={cardano.src} sx={{ width: "30%", background: "#7b61ff", borderRadius: "40%", padding: "6px" }}></Box> : item.blockchain === "ethereum" ? <Box component="img" src={eth.src} sx={{ width: "30%", background: "#7b61ff", borderRadius: "40%", padding: "6px" }}></Box> : item.blockchain === "binance" ? <Box component="img" src={binance.src} sx={{ width: "30%", background: "#7b61ff", borderRadius: "40%", padding: "6px" }}></Box> : item.blockchain === "polygon" ? <Box component="img" src={polygon.src} sx={{ width: "30%", background: "#7b61ff", borderRadius: "40%", padding: "6px" }}></Box> : item.blockchain === "solana" ? <Box component="img" src={solana.src} sx={{ width: "30%", background: "#7b61ff", borderRadius: "40%", padding: "6px" }}></Box> : ""
                                        }
                                    </Box>
            ...
            <Stack className={classes.createbuttonhead} alignItems={"center"} justifyContent={"center"}>
                <Button onClick={() => { onCreateNew() }} variant="contained" startIcon={<AddIcon className={classes.addicon} />} className={classes.createbutton} >Create New</Button>
            </Stack>
        </>
    )
}
